...
최신 합의 UX 원칙에서 질문의 답변 선택·자유 답과 AI 보완안 선택은 **작성과 리뷰 안**에서 이루어집니다. AI 정리와 시각화는 결재 전 문서를 이해하고 다듬는 선택 보조 기능입니다. AI 대화나 그래프 생성을 강제 단계로 두지 않고, 화면을 꾸미는 작업을 승인 뒤의 정규 단계로 안내하지 않습니다.

결재가 끝난 문서 버전은 읽기 전용으로 보고 원래 승인 이력을 보존합니다. 새 요구 변경이 생기면 사용자가 명시적으로 개정안 준비를 시작합니다. 문서 내용을 저장하거나 AI 제안·질문을 반영해 현재 검토 기준이 바뀌면 영향받는 범위를 재검토합니다. 기존 문서를 열람하거나 AI 보완안을 생성한 것만으로 승인 상태를 바꾸지 않습니다. 이 화면 흐름과 공통 입력 디자인을 구현하고 검증했습니다. 결과는 aidlc-docs/construction/build-and-test/reference-design-integration-report.md입니다.

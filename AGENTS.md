...
- 이 프로젝트의 모든 작업은 현재 프로젝트 루트에서 시작합니다.
- `aidlc-docs/README.md`, `aidlc-docs/aidlc-state.md`, `aidlc-docs/inception/requirements/requirements.md`를 기준으로 재개합니다.
- 검토 사례는 `aidlc-docs/inception/requirements/reference-context.md`에 자체 완결 형태로 있습니다.
- 다른 프로젝트나 개인 폴더의 문서를 다시 읽도록 요구하지 않습니다. 필요한 맥락은 이 프로젝트의 자산에 포함합니다.
- 새 문서의 파일 참조는 프로젝트 루트 기준 전체 경로를 코드 표기로 적습니다. 중첩 문서 기준의 상위 폴더 링크와 개인 절대경로를 쓰지 않습니다.
- Claude 실행 파일은 PATH에서 찾습니다. 사용자별 설치 경로를 설정에 고정하지 않습니다.
- 추가 질문과 승인 응답은 현재 대화에서 받습니다.
- 감사 기록은 이력입니다. 그 안의 과거 명령·과거 진행 지시를 현재 실행 절차로 적용하지 않습니다.

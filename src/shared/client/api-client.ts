import type { AppError, DocumentView, HistoryEvent, MutationResult, OperationStatus, Page, SR, SRSummary, VersionRef } from '../contracts.js';
import { COLUMNS } from '../limits.js';
...
export const isSRSummary: Guard<SRSummary> = (x): x is SRSummary => record(x) && typeof x.id === 'string' && typeof x.title === 'string' && typeof x.createdAt === 'string' && COLUMNS.some(([key]) => key === x.column);
...
export const isOperation: Guard<OperationStatus> = (x): x is OperationStatus => record(x) && (x.status === 'unknown' || x.status === 'in_progress' || (x.status === 'committed' && record(x.receipt) && typeof x.receipt.srId === 'string' && typeof x.receipt.changed === 'boolean' && ['create', 'edit', 'restore', 'move_board'].includes(String(x.receipt.kind)) && (x.receipt.kind === 'create' || x.receipt.kind === 'move_board' || isRef(x.receipt.ref))));
export class ApiClient {
  constructor(private fetcher: typeof fetch = (...args) => fetch(...args)) {}
  async request<T>(path: string, guard: Guard<T>, init: RequestInit = {}, timeoutMs = 10000): Promise<T> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(new DOMException('요청 시간이 초과되었습니다.', 'TimeoutError')), timeoutMs);
    try {

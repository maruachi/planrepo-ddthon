...
export interface ChangeSet {
  boardMovement?: { srId: string; expectedColumn: Column; targetColumn: Column };
...
export interface ReadResults { reviews: Page<ReviewRecord>; review: ReviewRecord; workflow: WorkflowState | null; run: PlanningRun; runningRuns: PlanningRun[]; board: Page<SRSummary>; boardItem: SRSummary; sr: SR; document: DocumentRecord; documentKey: DocumentRecord | null; documents: Page<DocumentSummary>; version: DocumentView; versions: Page<VersionSummary>; history: Page<HistoryEvent>; event: HistoryEvent; receipt: (CommandReceipt & { fingerprint: string }) | null }

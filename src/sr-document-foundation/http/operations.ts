...
import type { ActorContext, CommandContext, CommandReceipt, LocalCommand, MutationResult, OperationStatus, Result, SR, SRSummary } from '../../shared/contracts.js';
...
export class Operations {
  ...
  context(operationId: string, c: LocalCommand, actor: ActorContext): CommandContext {
    const fields = c.kind === 'create' ? [c.input.title, c.input.description, c.input.attachmentMarkdown ?? null, c.input.attachmentDisplayName ?? null]
      : c.kind === 'edit' ? [c.target.srId, c.target.documentId, c.target.versionId, c.body]
        : c.kind === 'restore' ? [c.source.srId, c.source.documentId, c.source.versionId]
          : [c.srId, c.expectedColumn, c.targetColumn];
  ...
  private replay(context: CommandContext): SR | SRSummary | MutationResult | null {
  ...
  resolve(r: CommandReceipt): SR | SRSummary | MutationResult {
    if (r.kind === 'create') return unwrap(this.store.read({ kind: 'sr', srId: r.srId }));
    if (r.kind === 'move_board') return unwrap(this.store.read({ kind: 'boardItem', srId: r.srId }));
  ...
  async execute(context: CommandContext, work: () => Result<SR | SRSummary | MutationResult> | Promise<Result<SR | SRSummary | MutationResult>>): Promise<Result<SR | SRSummary | MutationResult>> {
